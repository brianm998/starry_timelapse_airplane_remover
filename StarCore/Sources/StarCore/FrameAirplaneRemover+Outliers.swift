...
public class BlobProcessor {
    ...
    fileprivate func findBlobs() async throws -> BlobMap {
        ...
        let blobber: FullFrameBlobber = .init(config: frame.config,
    ...
    fileprivate func firstIsolatedDimProcess(blobs: BlobMap) async throws -> BlobMap {
        guard let frame else { return [:] }
        let idibr: DimIsolatedBlobRemover = .init(blobMap: blobs,
                                                  width: frame.width,
                                                  height: frame.height)
    ...
    fileprivate func firstIsolatedProcess(blobs: BlobMap) async throws -> BlobMap {
        guard let frame else { return [:] }
        let isolatedRemover: IsolatedBlobRemover = .init(blobMap: blobs,
                                                         width: frame.width,
                                                         height: frame.height)
    ...
    fileprivate func finalIsolatedRemover(blobs: BlobMap) async throws -> BlobMap {
        ...
        let finalIsolatedRemover: IsolatedBlobRemover = .init(blobMap: blobs,
                                                              width: frame.width,
                                                              height: frame.height)
    ...
    fileprivate func finalDimIsolatedRemover(blobs: BlobMap) async throws -> BlobMap {
        ...
        let dimIsolatedBlobRemover: DimIsolatedBlobRemover = .init(blobMap: blobs,
                                                                   width: frame.width,
                                                                   height: frame.height)
...
extension FrameAirplaneRemover {
    ...
    // XXX REALLY LONG METHOD TO DELETE
    public func loadOutliers_OLD() async throws {
    ...
    func findOutliers() async throws {
        ...
        // weed out blobs that are too small and not bright enough
        // XXX this is eating a lot of blobs we want :(
        let brighterBlobs: BlobMap = isolatedRemoverBlobs/*khtBlobs*/.compactMapValues { blob in
            if blob.adjustedSize < fx3Size(for: 1.14), // XXX constant
               blob.medianIntensity < 2000 // XXX constant
            {
                return blob
