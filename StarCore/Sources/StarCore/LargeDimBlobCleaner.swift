...
// cleans out dim pixels from large dim blobs
public actor LargeDimBlobCleaner {
    ...
    public struct Args: Sendable, Hashable, Equatable, Argable, Codable, Identifiable {
        ...
        let intensityFloor: UInt16 // median intensity pixels below this are removed
        ...
        public func description(for type: ArgType) -> String {
            ...
            case .intensityFloor:
                return "median intensity pixels below this are removed"
            }
        }
    ...
    public func process(_ args: Args) async {
        for (_, blob) in blobMap {
            ...
            {
                await blob.intensityTrim(by: args.intensityFloor)
            }
        }
    }
}
