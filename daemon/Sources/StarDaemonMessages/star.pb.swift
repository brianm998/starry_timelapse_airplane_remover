...
public struct Star_V1_Config: @unchecked Sendable {
  ...
  /// Divide each frame's dimensions by this before detecting keypoints. 1 = full
  /// resolution, 2 = half size, 1.5 = two thirds. Detection time and peak memory both
  /// fall as 1/divisor^2; what it costs is keypoint localisation precision, which shows
  /// up as softness in the merged result. Values <= 1 mean full resolution.
  public var alignmentKeypointDetectionDivisor: Double {
    get {_storage._alignmentKeypointDetectionDivisor ?? 0}
    set {_uniqueStorage()._alignmentKeypointDetectionDivisor = newValue}
  }
  /// Returns true if `alignmentKeypointDetectionDivisor` has been explicitly set.
  public var hasAlignmentKeypointDetectionDivisor: Bool {_storage._alignmentKeypointDetectionDivisor != nil}
  /// Clears the value of `alignmentKeypointDetectionDivisor`. Subsequent reads from it will return its default value.
  public mutating func clearAlignmentKeypointDetectionDivisor() {_uniqueStorage()._alignmentKeypointDetectionDivisor = nil}
