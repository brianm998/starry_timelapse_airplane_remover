...
my ($airplane_value, $airplane_mid_value) = process_csv_dir($airplane_dir);
my ($non_airplane_value, $non_airplane_mid_value) = process_csv_dir($non_airplane_dir);
...
open my $output, ">Sources/ntar/ShouldPaint.swift";

print $output <<END
import Foundation

// XXX do not edit this file by hand, it's generated by $0
...
func should_paint_based_upon(lines: [Line]) -> Bool {
    ...
    if let mid_value = mid_value {
        if counts_over_lines < $keys_over_lines_avg,
	   mid_value < $mid_index_avg
	{
	    return true
        }
    }
...
#      print "$filename has $line_count lines and $key_count keys $first_count first count $keys_over_lines percentage\n";
  ...
  return ($total_keys_over_lines, $average_mid_value);
}
