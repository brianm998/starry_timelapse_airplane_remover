...
class DecisionTreeGenerator {
        ...
        let swift_string = """
          /*
             ...
             with test data consisting of:
               - \(positive_test_data.count) groups known to be paintable
               - \(negative_test_data.count) groups known to not be paintable
                                      ...
                                      indent: Int) async -> SwiftDecisionTree
    {
        ...
        let should_paint_values = await transform(testData: positive_test_data)
        let should_paint_dist = await getValueDistributions(of: should_paint_values)
        
        // indexed by outlierGroup.sortOrder
        let should_not_paint_values = await transform(testData: negative_test_data)
