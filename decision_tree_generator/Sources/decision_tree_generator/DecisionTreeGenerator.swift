...
actor DecisionTreeGenerator {
    ...
    func generateTree(withPositiveData positive_training_data: [OutlierGroupValueMap],
                      ...
                      baseFilename: String) async throws -> DecisionTreeStruct
    {
        ...
        digest.update(data: Data("\(positive_training_data.count)".utf8))
        digest.update(data: Data("\(negative_training_data.count)".utf8))
        ...
        let tree = await decisionTreeNode(with: positive_training_data,
                                          and: negative_training_data,
        ...
        let swift_string = """
          /*
             ...
             with test data consisting of:
               - \(positive_training_data.count) groups known to be positive
               - \(negative_training_data.count) groups known to be negative
  ...
  async -> TreeDecisionTypeResult
{
    ...
    let positive_training_data_count = positive_training_data.count
    for index in 0..<positive_training_data_count {
        let group_values = positive_training_data[index]
    ...
    let negative_training_data_count = negative_training_data.count 
    for index in 0..<negative_training_data_count {
        let group_values = negative_training_data[index]
...
@available(macOS 10.15, *) 
fileprivate func decisionTreeNode(with positive_training_data: [OutlierGroupValueMap],
                                  and negative_training_data: [OutlierGroupValueMap],
                                  indented indent: Int,
                                  decisionTypes: [OutlierGroup.TreeDecisionType],
                                  decisionSplitTypes: [DecisionSplitType],
                                  maxDepth: Int)
  async -> SwiftDecisionTree
{
    ...
    let positive_task = await runTask() {
        ...
        let positiveValues = await transform(testData: positive_training_data, on: decisionTypes)
        return await getValueDistributions(of: positiveValues, on: decisionTypes)
    }
    
    let negative_task = await runTask() {
        // indexed by outlierGroup.sortOrder
        let negativeValues = await transform(testData: negative_training_data, on: decisionTypes)
